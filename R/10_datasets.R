# ==============================================================================
# 10_datasets.R  (refactor of legacy 00_config_data.R)
# Builds the DATASETS registry (fc / bo / ro / cross) with context variables
# (SES_z, OF02_01_num_log, OF02_02_num_log, OF_Spender) from daten_standardisiert.
# Paths come from R/00_paths.R (portable) — no hard-coded user paths.
# ==============================================================================

if (!exists("DATA_DIR")) source(here::here("R", "00_paths.R"))
...
# Paths are provided by R/00_paths.R: CSV_DIR, SYNTAX_DIR, OUT_DIR, etc.
...
DATA_FILE <- DATEN_STANDARDISIERT

if (!file.exists(DATA_FILE)) {
  stop(
    "Data file not found: ", DATA_FILE,
    "\nSet CBE_DATA_DIR or place the file at data/derived/daten_standardisiert.RData"
  )
}
...
readr::write_csv(
  source_inventory,
  file.path(CSV_DIR, "00_source_inventory.csv"),
  na = ""
)
...
readr::write_csv(
  source_scores,
  file.path(CSV_DIR, "00_source_scores.csv"),
  na = ""
)
...
readr::write_csv(
  dataset_audit,
  file.path(CSV_DIR, "01_dataset_audit.csv"),
  na = ""
)
# ------------------------------------------------------------------------------
